...
class PriceInput(DTO):
    ...
    *Quote* token to determine the value of the base token.
    ...
    class Config:
        schema_extra = {
            "examples": [
                {"base": {"symbol": "CRV"}, "_test_multi": {"chain_id": 1}},
                {"base": {"symbol": "USD"}},
                {
...
class PriceInputWithPreference(PriceInput):
    ...
    try_other_chains: bool = DTOField(
        False,
        description="If prices are not found on the input chain, try other chains for prices.",
    )
...
class PriceHistoricalInput(PriceInputWithPreference, MapBlockTimeSeriesInput):
    ...
    class Config:
        schema_extra = {
            "examples": [
                {"base": {"symbol": "AAVE"}, "interval": 86400, "count": 1, "exclusive": True}
            ]
        }
...
class PriceWeight(DTO):
    ...
    class Config:
        schema_extra = {"examples": [{"weight_power": 4.0}]}
...
class DexPoolPriceInput(PriceWeight, DexProtocolInput):
    price_slug: str
    ref_price_slug: Optional[str] = DTOField(
        description="Set to None for not using reference price"
    )
                    ...
                    "some": [
                        {
                            ...
                            "price0": 0.9528107174897676,
                            "price1": 1.0495263976821705,
                            "one_tick_liquidity0": 0.001685478240180313,
                            "one_tick_liquidity1": 0.0016060220264204467,
                            "full_tick_liquidity0": 33.712093,
                            "full_tick_liquidity1": 32.12124351941178,
                            "token0_address": "0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48",
                            "token1_address": "0xd533a949740bb3306d119cc777fa900ba034cd52",
                            "token0_symbol": "USDC",
                            "token1_symbol": "CRV",
                            "pool_address": "0x210a97ba874a8e279c95b350ae8ba143a143c159",
                            "ref_price": 1.0006473790684685,
                            "tick_spacing": 1,
                        },
                        {
                            "src": "uniswap-v2.get-weighted-price",
                            "price0": 0.9511103469743591,
                            "price1": 1.0514027138713893,
                            "one_tick_liquidity0": 0.0003462790926172877,
                            "one_tick_liquidity1": 0.00032936609499925525,
                            "full_tick_liquidity0": 6.926101266677137,
                            "full_tick_liquidity1": 6.58748657892884,
                            "token0_address": "0x6b175474e89094c44da98b954eedeac495271d0f",
                            "token1_address": "0xd533a949740bb3306d119cc777fa900ba034cd52",
                            "token0_symbol": "DAI",
                            "token1_symbol": "CRV",
                            "pool_address": "0xf00f7a64b170d41789c6f16a7eb680a75a050e6d",
                            "ref_price": 0.99962940609268,
                            "tick_spacing": 1,
                        },
                    ],
                    "weight_power": 4.0,
...
PRICE_DATA_ERROR_DESC = ModelDataErrorDesc(
    code=ModelDataError.Codes.NO_DATA, code_desc="No pool to aggregate for token price"
)
            ...
            "examples": [
                {
                    "base": {"symbol": "CRV"},
                    "quote": {"symbol": "USD"},
                    "block_numbers": [16_000_000, 16_001_000],
                },
            ]
        }
