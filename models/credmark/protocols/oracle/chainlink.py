...
@Model.describe(
    slug="chainlink.get-feed-registry",
    version="1.0",
...
class ChainLinkPriceByFeed(Model):
    def run(self, input: ChainlinkFeedContract) -> Price:
        ...
        try:
            ...
            feed_contract = feed_contract.set_abi(CHAINLINK_AGG, set_loaded=True)
        ...
        return Price(
            ...
            src=(
                f"{self.slug}|{description}|{feed}|v{version}|"
...
@Model.describe(
    slug="chainlink.price-from-registry-maybe",
    version="1.4",
    ...
    def run(self, input: PriceInputWithRegistry) -> Maybe[PriceWithQuote]:
        try:
            pq = self.context.run_model(
                "chainlink.price-by-registry", input=input, return_type=PriceWithQuote, local=True
            )
...
@Model.describe(
    slug="chainlink.price-by-registry",
    version="1.7",
    display_name="Chainlink - Price by Registry",
    description="Looking up Registry for two tokens' addresses",
    category="protocol",
...
class ChainLinkPriceByRegistry(Model):
    def run(self, input: PriceInputWithRegistry) -> Price:
        ...
        try:
            ...
            [round_data, decimals, description, version, is_feed_enabled] = (
                self.context.web3_batch.call(
                    [
            ...
            (_roundId, answer, _startedAt, _updatedAt, _answeredInRound) = cast(
                tuple[int, int, int, int, int], round_data
            )
