...
class CompoundV2Meta(Model):
    ...
    def get_comptroller(self):
        addr = self.COMPTROLLER[self.context.network]
...
@Model.describe(slug="compound-v2.get-comptroller",
                ...
                output=Contract)
class CompoundV2Comptroller(CompoundV2Meta):
    # pylint:disable=locally-disabled,protected-access
    def run(self, __) -> Contract:
        comptroller = self.get_comptroller()
...
@Model.describe(slug="compound-v2.get-pools",
                ...
                subcategory='compound',
                output=Some[Address])
class CompoundV2GetAllPools(CompoundV2Meta):
    def run(self, _) -> Some[Address]:
        comptroller = self.get_comptroller()
        cTokens = cast(list[str], comptroller.functions.getAllMarkets().call())

        # Check whether our list is complete
        # assert ( sorted([Address(x) for x in CTOKENS.values()]) ==
...
class CompoundV2GetPoolInfo(CompoundV2Meta):
    """
    # Pool info
    ...
    def test_fixture(self, chain_id):
        assets = sorted(self.ASSETS[chain_id].keys())
        compound_ctokens = sorted(['WETH' if t == 'cETH' else t[1:]
                                  for t, _ in self.CTOKENS[chain_id].items()])
        assert assets == compound_ctokens
    ...
    def run(self, input: CompoundV2Token) -> CompoundV2PoolInfo:
        ...
        assert cToken.functions.admin().call() == Address(
            self.TIMELOCK[self.context.network])
