...
class CompoundGetAssets(credmark.model.Model):
    ...
    def run(self, input: AddressDTO) -> dict:
        ...
        comp_asset = contract.functions.markets(Address(input.address)).call() #converting the address to 'Address' type for safety
        tokencontract = self.context.web3.eth.contract(address=input.address, abi= ERC_20_TOKEN_CONTRACT_ABI)
