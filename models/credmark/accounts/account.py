...
class AccountReturnInput(Account):
    ...
    quote: Currency = DTOField(
        FiatCurrency(symbol="USD"), description="Currency of the returned price if included."
    )
    ...
    class Config:
        ...
        schema_extra = {
            "examples": [
                {"address": "0x388C818CA8B9251b393131C08a736A67ccB19297", "token_list": "cmf"}
            ]
        }


@Model.describe(
    slug="account.token-return",
    version="0.7",
...
class AccountERC20TokenReturn(Model):
    def run(self, input: AccountReturnInput) -> TokenReturnOutput:
        return self.context.run_model(
            "accounts.token-return",
            input=AccountsReturnInput(
                accounts=[Account(input.address)], token_list=input.token_list, quote=input.quote
            ),
...
class AccountsTokenReturn(Model):
    def run(self, input: AccountsReturnInput) -> TokenReturnOutput:
        ...
        for account in input.accounts:
            _native_amount += native_token.balance_of_scaled(account.address.checksum)
        ...
        if not df_native.empty:
            ...
            df_native.insert(
                df_native.shape[1],
                "value",
                df_native["native_amount"] + df_native["gas_used"] + df_native["internal_tx"],
            )
            df_native_comb = df_native.loc[
                :,
                [
                    "block_number",
                    "log_index",
    ...
    class Config:
        schema_extra = {
            "examples": cross_examples(
                AccountReturnInput.Config.schema_extra["examples"],
...
@Model.describe(
    slug="account.token-return-historical",
    version="0.1",
...
class AccountsReturnHistoricalInput(AccountsReturnInput, HistoricalDTO):
    class Config:
        schema_extra = {
            "examples": cross_examples(
                AccountsReturnInput.Config.schema_extra["examples"],
                HistoricalDTO.Config.schema_extra["examples"],
                limit=10,
            )
        }
...
# TODO: NFT
@Model.describe(
    slug="accounts.token-return-historical",
    version="0.10",
...
class AccountsTokenReturnHistorical(Model):
    def run(self, input: AccountsReturnHistoricalInput) -> MapBlockTimeSeriesOutput[dict]:
        ...
        if input.token_list == TokenListChoice.CMF:
            token_list = (
                self.context.run_model("token.list", {}, return_type=Records)
                .to_dataframe()["address"]
                .str.lower()
                .values
            )
        else:
        ...
        for n_historical, row in df_historical.iterrows():
            ...
            assets = []
            _native_token_bal = (
                _df_native.query("(block_number <= @_past_block_number)")
            ...
            def _use_model(_assets, _token_bal, _past_block_number):
                non_zero_bal_tokens_dict = {
                    token_bal_row["token_address"]: token_bal_row["value"]
                ...
                for p_maybe, (token_addr, token_value) in zip(
                    pqs_maybe.some, non_zero_bal_tokens_dict.items()
                ):
                    ...
                    try:
                        _assets.append(
                            Position(amount=asset_token.scaled(token_value), asset=asset_token)
                        )
            ...
            def _use_for(_assets, _token_bal, _past_block_number):
                for _, token_bal_row in _token_bal.iterrows():
                    if token_bal_row["value"] > 0 and not math.isclose(token_bal_row["value"], 0):
                        ...
                        try_price = self.context.run_model(
                            "price.quote-maybe",
            ...
            price_historical_result[n_historical].output = {
                "value": Portfolio(positions=assets).get_value(block_number=_past_block_number)
            }
...
class AccountsERC20TokenHistorical(Model):
    def account_token_historical(self, input: AccountsHistoricalInput, do_wobble_price: bool):
        ...
        price_historical_result = MapBlocksOutput[dict](**balance_result)

        if _include_price:
            all_blocks = list(
                functools.reduce(lambda a, b: a | set(b), token_blocks.values(), set())
            )
            ...
            for i in range(0, len_list, chunk_size):
                ...
                self.logger.info(
                    "Fetching `price.dex-db-blocks-tokens` "
            ...
            for token_addr, past_blocks in token_blocks.items():
                ...
                if (
                    prices is None
                    or len(prices) == 0
                    or len([p for p in prices if p["price"] is None]) > 0
                ):
                    continue
...
class AccountsERC20TokenHistoricalBalance(Model):
    def run(self, input: AccountsHistoricalInput) -> dict:
        ...
        for n_historical, row in enumerate(input.get_blocks()):
            ...
            results.append(
                MapBlockResult(blockNumber=BlockNumber.from_dict(row.dict()), output=None)
            )
            assets = []
            native_amount = 0
            ...
            if not math.isclose(native_amount, 0):
                assets.append(
                    Position(amount=native_token.scaled(native_amount), asset=native_token)
                )

            token_bal = (
                df_erc20.query("(block_number <= @past_block_number)")
                .groupby("token_address", as_index=False)["value"]
                .sum()
...
class AccountWithPrice(Account):
    ...
    def to_accounts(self):
        return AccountsWithPrice(
            accounts=[self.address],  # type: ignore
...
class AccountPortfolio(Model):
    def run(self, input: AccountWithPrice) -> Portfolio:
        return self.context.run_model(
            "accounts.portfolio", input=input.to_accounts().dict(), return_type=Portfolio
        )


@Model.describe(
    slug="accounts.portfolio",
    version="0.7",
    ...
    def run(self, input: AccountsWithPrice) -> Portfolio:
        ...
        for address, token_addresses in tokens_by_wallet.items():
            ...
            balances = self.context.web3_batch.call_same_function(
                fn, token_addresses, unwrap=True, unwrap_default=0
            )
        ...
        if self.context.network is Network.Mainnet and input.include_curve_positions:
            curve_lp_position = self.context.run_model("curve.lp-accounts", input=input)
        ...
        pqs_maybe = self.context.run_model(
            slug="price.quote-multiple-maybe",
            input=Some(some=[{"base": p.asset.address, "quote": input.quote} for p in positions]),
            return_type=Some[Maybe[PriceWithQuote]],
        )

        price_positions = []
        for price_maybe, position in zip(pqs_maybe.some, positions):
            price_quote = price_maybe.get_just(
                PriceWithQuote(price=0.0, src="none", quoteAddress=input.quote.address)
            )
            price_positions.append(
                Position(amount=position.amount, asset=position.asset, price_quote=price_quote)
            )
...
class AccountBalances(Model):
    def run(self, input: AccountBalancesInput) -> Portfolio:
        return self.context.run_model(
            "accounts.balances", input=input.to_accounts().dict(), return_type=Portfolio
        )
...
class AccountsBalances(Model):
    def get_token_decimals(self, tokens: list[ChecksumAddress]):
        ...
        decimals = self.context.web3_batch.call_same_function(
            fn, list(tokens), fallback_functions=[fn_fallback], unwrap=True, unwrap_default=0
        )
    ...
    def include_price(self, positions: List[Position], quote: Currency) -> List[Position]:
        pqs_maybe = self.context.run_model(
            slug="price.quote-multiple-maybe",
            input=Some(some=[{"base": p.asset.address, "quote": quote} for p in positions]),
            return_type=Some[Maybe[PriceWithQuote]],
        )
        ...
        for price_maybe, position in zip(pqs_maybe.some, positions):
            price_quote = price_maybe.get_just(
                PriceWithQuote(price=0.0, src="none", quoteAddress=quote.address)
            )
            price_positions.append(
                Position(amount=position.amount, asset=position.asset, price_quote=price_quote)
            )
    ...
    def run(self, input: AccountsBalancesInput) -> Portfolio:
        ...
        if native_token is not None:
            ...
            balances[str(native_token.address)] = Position(amount=native_amount, asset=native_token)
        ...
        for account in input.accounts:
            ...
            balances = self.context.web3_batch.call_same_function(
                fn, tokens, unwrap=True, unwrap_default=0
            )
