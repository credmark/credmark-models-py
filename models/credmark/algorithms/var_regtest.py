...
TEST_CASE = {
    'finance.var-reference': TEST_CASE_VAR,
            ...
            for case_name, case_detail in cases.items():
                ...
                if dev_mode:
                    os.rename(os.path.join('tmp', 'df_res.csv'),
                              os.path.join('tmp', f'df_res_{model_slug}.{case_name}.csv'))
                    if model_slug == 'finance.var-reference':
                        os.rename(os.path.join('tmp', 'df_hist.csv'),
                                  os.path.join('tmp', f'df_hist_{model_slug}.{case_name}.csv'))
                    with open(os.path.join('tmp', f'{model_slug}.{case_name}.json'), 'w') as f:
                        f.write(res.__str__())

                regtest_file = os.path.join('tmp', f'regtest_{model_slug}.{case_name}.json')
