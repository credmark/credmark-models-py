...
def calc_var(ppl, lvl) -> VaROutput:
    ...
    var = np.percentile(ppl_sorted, lvl*100,
    ...
    if where_var.shape[0] == 0:
        ...
        unsorted_index = np.where(
            np.isin(ppl, ppl_sorted[sorted_index]))[0].tolist()
...
def calc_es(ppl, lvl) -> ESOutput:
    ...
    return ESOutput(es=es,
                    ...
                    weights=[1.0] * var_result.sorted_index[-1] +
                    [var_result.weights[-1]],
...
if __name__ == '__main__':
    ...
    assert np.isclose(calc_var(test_ppl, 0.01).var,
