...
class SharpRatioInput(DTO):
    token: Token
    return_rolling_days: int = DTOField(182, ge=2)
...
@Model.describe(slug="finance.sharpe-ratio-token",
                ...
                description=("Sharpe ratio is return (averaged daily returns, annualized) "
...
class SharpeRatioToken(Model):
    ...
    def run(self, input: SharpRatioInput) -> dict:
        token = input.token
        return_rolling_days = input.return_rolling_days
        window = f'{int(input.return_rolling_days*2)} days'
        ...
        avg_6m_ret = (pd.Series(annualized_return)
                      .rolling(return_rolling_days)
                      .mean()[return_rolling_days-1:])

        st_dev = (avg_6m_ret.rolling(return_rolling_days)
                  .std()
                  [(return_rolling_days-1):])

        sharpe_ratio = (avg_6m_ret[return_rolling_days-1:] - risk_free_rate) / st_dev
