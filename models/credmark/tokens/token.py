...
def get_eip1967_proxy(context, logger, address, verbose):
    ...
    if not proxy_address.is_null():
        ...
        if (
            token._meta.is_transparent_proxy
            and token.proxy_for is not None
            and proxy_address != token.proxy_for.address
        ):
            logger.debug(
...
def get_eip1967_proxy_err_with_abi(
    context, logger, address, verbose, proxy_abi, implementation_abi
):
    ...
    proxy_contract._meta.proxy_implementation = Contract(proxy_contract.proxy_for.address).set_abi(
        implementation_abi, set_loaded=True
    )
...
class TokenDeploymentOutput(ImmutableOutput):
    deployed_block_number: BlockNumber = DTOField(description="Block number of deployment")
    deployed_block_timestamp: Optional[int] = DTOField(description="Timestamp of deployment")
    deployer: Optional[Address] = DTOField(description="Deployer address")
    proxy_deployer: Optional[dict] = DTOField(description="Proxy deployment")


@Model.describe(
    slug="token.deployment-maybe",
    version="0.12",
    ...
    def run(self, input: TokenDeploymentInput) -> Maybe[TokenDeploymentOutput]:
        try:
            res = self.context.run_model(
                "token.deployment", input, return_type=TokenDeploymentOutput
            )
    ...
    def run(self, input: TokenDeploymentInput) -> TokenDeploymentOutput:
        ...
        if self.context.chain_id == Network.Mainnet and not input.ignore_proxy:
            if input.proxy_for is not None:
                proxy_deployer = self.context.run_model(self.slug, input.proxy_for)
...
class TokenLogoModel(Model):
    ...
    logos = {
        "trustwallet_assets": NetworkDict(
            str,
            {
    ...
    def run(self, input: Token) -> TokenLogoOutput:
        ...
        if self.context.network in logos["sushiswap_logos"]:
            try_urls.append(
                (
                    "https://raw.githubusercontent.com/sushiswap/list/master/logos/token-logos"
...
class TokenTransferInput(Token):
    ...
    offset: int = DTOField(
        0, ge=0, description="Omit a specified number of transfers from beginning of result set"
    )
...
class TokenTransferOutput(IterableListGenericDTO[TokenTransfer]):
    transfers: List[TokenTransfer] = DTOField(default=[], description="List of transfers")
...
class TokenHolders(Model):
    def run(self, input: TokenHolderInput) -> TokenHoldersOutput:
        with self.context.ledger.TokenBalance as q:
            ...
            token_price_maybe = Maybe[PriceWithQuote](
                **self.context.models.price.quote_maybe({"base": input, "quote": input.quote})
            )
        ...
        schema_extra = {
            "example": {
                "categories": [
                    {
                        "accounts": {
                            "accounts": [{"address": "0x1F98431c8aD98523631AE4a59f267346ea31F984"}]
                        },
                        "categoryName": "",
                        "categoryType": "",
                        "circulating": True,
...
class TokenCirculatingSupply(Model):
    def run(self, input: CategorizedSupplyRequest) -> CategorizedSupplyResponse:
        ...
        response.categories.append(
            CategorizedSupplyResponse.CategorizedSupplyCategory(
...
class TokenAllInput(DTO):
    ...
    class Config:
        schema_extra = {"example": {"limit": 10, "page": 2}}
...
class TokenAll(Model):
    def run(self, input: TokenAllInput) -> TokenAllOutput:
        with self.context.ledger.Token as token:
            rows = token.select(
                aggregates=[(token.ADDRESS.count_(), "count_token_address")],
        ...
        return TokenAllOutput(
            total=count_token_address,
            limit=input.limit,
            page=input.page,
