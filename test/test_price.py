...
def test_chainlink(self, _token_addr: str, _price_model: str, _prefer: str):
    ...
    if _token_addr.startswith('0x'):
        token_input = {"address": _token_addr}
    else:
        token_input = {"symbol": _token_addr}

    self.run_model(_price_model, {"base": token_input,
                                  "quote": {"symbol": "USD"}, "prefer": _prefer},
                   block_number=block_number)
    self.run_model(_price_model, {"quote": token_input,
                                  "base": {"symbol": "USD"}, "prefer": _prefer},
                   block_number=block_number)
    self.run_model(_price_model, {"base": token_input,
                                  "quote": {"symbol": "JPY"}, "prefer": _prefer},
                   block_number=block_number)
    self.run_model(_price_model, {"quote": token_input,
                                  "base": {"symbol": "JPY"}, "prefer": _prefer},
                   block_number=block_number)
    self.run_model(_price_model, {"base": token_input,
                                  "quote": {"symbol": "GBP"}, "prefer": _prefer},
                   block_number=block_number)
    self.run_model(_price_model, {"quote": token_input,
                                  "base": {"symbol": "GBP"}, "prefer": _prefer},
                   block_number=block_number)
    self.run_model(_price_model, {"base": token_input,
                                  "quote": {"symbol": "EUR"}, "prefer": _prefer},
                   block_number=block_number)
    self.run_model(_price_model, {"quote": token_input,
                                  "base": {"symbol": "EUR"}, "prefer": _prefer},
                   block_number=block_number)
    self.run_model(_price_model, {"base": token_input,
                                  "quote": {"address": "0xD31a59c85aE9D8edEFeC411D448f90841571b89c"},
...
def run_test_price_mix(self, tok):
    block_number = 15000108
    for _prefer in ['cex', 'dex']:
        if prefer == 'dex' and tok not in CEX_ONLY_TOKENS:
            self.run_model('price.quote',
        ...
        if prefer == 'cex':
            self.run_model('price.quote', {"quote": {"address": tok}, "base": {
                "symbol": "USD"}, "prefer": _prefer},
                block_number=block_number)
            self.run_model('price.quote',
